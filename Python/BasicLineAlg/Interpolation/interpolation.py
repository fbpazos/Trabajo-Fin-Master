...
def interPoly(x,y,u):
    ...
    '''
    n = len(x)
    v = np.zeros(len(u)) # Create a vector of zeros of the same size as u, to store the result
    for i in range(n):
        w = np.ones(len(u)) 
        for j in range(n):
            if j != i:
...
def piecewiseLinear(x,y,u,sorted=False):
    ...
    delta = np.diff(y)/np.diff(x) # Compute the slopes of the lines -- here we are using the fact that x is sorted 
    n = len(x) 
    
  
    k = np.ones(len(u)).astype(int) # Create a vector of ones of the same size as u, to store the result
    for i in range(1,n):
        k[x[i-1] <= u] = int(i) # Find the index of the points in u that are between x[i] and x[i+1] and store the index in k 
   

    s = [u[i] - x[k[i]-1] 
            for i in range(len(u))] # Compute the distance between the points in u and the points in x that are between x[i] and x[i+1]

    v = [y[k[i]-1] + s[i]*delta[k[i]-1] 
            for i in range(len(u))] # Compute the value of the interpolation at the points in u
...
def pchip(x,y,u,sorted=False):
    ...
    def pchip_slopes(h,delta):
        ...
        w1 = 2*h[k] + h[k-1]
        w2 = h[k] + 2*h[k-1]
        d[k] = (w1+w2)/(w1/delta[k-1] + w2/delta[k]) # Compute the slopes of the lines
