...
def interactive_lu(P, L, U, col, pivotRow):
    ...
    if U[iMax, col] != 0:
        ...
        P = permute(P, iMax, col)
        L = permute(L - np.eye(L.shape[0]), iMax, col) + np.eye(
            L.shape[0]
        )  # We cant permute L without changing the values of the diagonal, so we permute L-I and add I to get the same effect
        ...
        for row in range(col + 1, U.shape[0]):
            ...
            U[row, col] = 0
            U[row, col + 1 :] = (
                U[row, col + 1 :] - L[row, col] * U[col, col + 1 :]
            )  # Update the remaining elements in the row using the multiplier
