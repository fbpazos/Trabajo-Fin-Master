    ...
    def initializeComponents(self):
        ...
        for key, val in self.methods.items():
            checkbox = widgets.Checkbox(
                description=key, value=True, style={"background": val[1]}
            )
...
class InterpolVisualizer_UnOptimized:
    ...
    def initializeComponents(self):
        ...
        self.checkboxes = [
            widgets.Checkbox(
                description="InterPoly", value=True, style={"background": "lightblue"}
            ),
            widgets.Checkbox(
            ...
            widgets.Checkbox(
                description="Pchip", value=True, style={"background": "bisque"}
            ),
            widgets.Checkbox(
                description="SplitNetX", value=True, style={"background": "violet"}
            )
    ...
    def scattering(self):
        ...
        """
        self.scatterPlot = plt.scatter(
            self.x, self.y, colors=["red"], stroke="red", label="Points"
        )
    ...
    def update_pointsY(self, change):
        ...
        self.y = (
            ...
            if change is not None
            and change["name"] == "y"
            and len(self.x) == len(list(change["new"]))
            else self.y
        )
    ...
    def interpolate(self):
        ...
        if self.checkboxes[0].value:
            plt.plot(
                self.u,
                interPoly(self.x, self.y, self.u),
                colors=["blue"],
                stroke="blue",
                label="InterPoly",
            )
        if self.checkboxes[1].value:
            plt.plot(
                self.u,
                piecewiseLinear(self.x, self.y, self.u),
                colors=["green"],
                stroke="green",
        ...
        if self.checkboxes[3].value:
            plt.plot(
                self.u,
                splitnetx(self.x, self.y, self.u),
                colors=["purple"],
                stroke="purple",
                label="SplitnetX",
            )
